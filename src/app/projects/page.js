  ...
  return (
    <main className="max-w-5xl mx-auto p-6">
      ...
      <div className="mt-6 grid gap-4 md:grid-cols-2">
        {(projects ?? []).map((p) => {
          const publishedHref = p.liveUrl || p.githubUrl; // published site first, then GitHub
          const detailHref = `/projects/${p.slug}`;

          const href = publishedHref || detailHref;
          const isExternal = Boolean(publishedHref);

          const CardInner = (
            <>
              <h2 className="text-lg font-semibold underline">{p.title}</h2>
          ...
          return (
            <article key={p._id} className="rounded-2xl border p-4">
              {isExternal ? (
                ...
                <Link href={href} className="block" aria-label={`Open ${p.title}`}>
                  {CardInner}
                </Link>
              )}
            </article>
